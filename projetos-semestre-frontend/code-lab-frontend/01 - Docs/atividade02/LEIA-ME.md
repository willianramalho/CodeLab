...
1. `print-curl-search.jpg` — print do terminal rodando
   `curl "http://localhost:3000/api/search?q=teste"` respondendo corretamente.
2. `print-console-busca-ok.jpg` — print do Console do navegador (F12) mostrando
   `Busca OK:` com o objeto `{ query: 'a', challenges: [], users: [] }`.
3. `print-erro-rede.jpg` — print do erro de rede provocado de propósito (API
